...
使用回溯枚举所有情况，然后找出满足条件的最大值。回溯算法的复杂度为 O(2^n)，指数级别。
...
通过图片可以发现规律，图中 f(2, 2) 和 f(3,4) 都被重复计算了两次。可以利用备忘录的方式，避免第二次重复计算。
...
上面程序还可以优化，把states变成一维数组，降低空间复杂度。[01-2.go](https://github.com/lzle/algorithm/blob/master/algorithm/dynamic/01-2.go)
